---
name: api-guide
description: OCR 서비스 API 가이드 문서를 생성하거나 수정한다. API 가이드 추가, 버전별 문서 분리, 인증 방식 변경, 공통 정보 섹션 작성, 다국어 문서 생성에 사용한다. "API 가이드 작성", "OCR 가이드 추가", "새 버전 분리", "v2.0 / v2.1 가이드", "신규 인증 방식 가이드", "공통 정보 섹션", "신규 API 엔드포인트 문서화" 같은 요청이 보이면 즉시 이 스킬을 사용한다.
---
...
- **ko가 단일 source of truth**: 한국어 가이드를 먼저 확정하고 다국어로 동기화한다. 다국어 변경이 한국어보다 앞서면 정합성이 깨진다.
- **zh = en 동일 본문**: 중국어 가이드는 영어 본문을 그대로 사용하는 운영 정책. zh 전용 표현/번역을 새로 만들지 않는다.
- **버전 분리(v1.x vs v1.1/v2.1)**: 같은 서비스라도 인증 방식이 갈라지면 별도 가이드로 분리한다. v1.0/v2.0은 Appkey+SecretKey, v1.1/v2.1은 User Access Key Token 기반이라 사용자 혼동 방지를 위해 가이드 자체를 분리.
- **버전 숫자만 갈리고 URI 구조는 동일**하게 유지하여 마이그레이션 비용을 낮춘다.
