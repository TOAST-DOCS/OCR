...
* **신뢰 값 제공**
    * 이미지에서 추출한 텍스트의 신뢰 값(confidence)을 제공합니다.
* **분석 결과 다운로드**
...
* 파일/이미지 URL 권장 사항
    ...
    * 최대 용량: 20MB
    * 권장 해상도: 1280x720
    * 긴 이미지를 분석하는 경우 '분할 인식 기능'으로 더 정확한 분석 결과를 제공받을 수 있습니다.
        * '가로-세로' 또는 '세로-가로' 종횡 비율이 1:2.5를 넘는 긴 이미지는 분석 성능 저하의 원인이 될 수 있습니다.
        * 종횡 비율이 1:2.5를 초과할 때 정확한 인식이 필요하다면 '분할 인식 기능'으로 분석 결과의 정확도를 높일 수 있습니다.
            * 이미지를 분할하여 인식할 때 일부 중첩된 영역에서 중복된 텍스트가 분석 결과에 포함될 수 있습니다.
        * 종횡 비율이 약 1:10을 넘으면 분석할 수 없습니다.
* 이미지 권장 사항
    * 평평한 곳에서 최대한 바르게 펴진 상태로 촬영된 이미지를 사용하세요.
    * 사각 형태의 가득 찬 이미지를 사용하세요.
    * 카메라 플래시 등에 의한 빛 반사나 그늘로 글자가 잘 안 보이는 경우, 해상도 대비 문자 크기가 작은 경우에는 정확한 문자 추출이 어려울 수 있습니다.
...
* **사업자등록증 주요 데이터 추출 및 분석**
    * 사업자등록증 분류(개인/법인)에 따른 주요 데이터는 Key/Value 한 쌍으로 분석되며, 주요 데이터의 신뢰 값(confidence)을 제공합니다.
* **분석 결과 다운로드**
...
* 파일 권장 사항
    ...
    * 최대 용량: 20MB
    * 권장 해상도: 1280x720 이상
* PDF의 경우 단일 페이지 분석 결과만 제공합니다(여러 페이지인 경우 첫 페이지 분석 결과를 제공합니다).
* 평평한 곳에서 최대한 바르게 펴진 상태로 촬영된 이미지를 사용하세요.
* 사각 형태의 가득 찬 이미지를 사용하세요.
* 카메라 플래시 등에 의한 빛 반사나 그늘로 글자가 잘 안 보이는 경우 정확한 Key/Value 추출이 어려울 수 있습니다.
* 흑백/컬러 이미지 모두 결과 분석이 가능하지만, 정확한 분석에는 컬러 이미지를 권장합니다.
...
* **신용카드 주요 데이터 추출 및 분석**
    * 신용카드 이미지의 카드 번호, 유효 기간 정보와 해당 정보의 신뢰 값(confidence)을 제공합니다.
* **분석 결과 다운로드**
...
* 카메라 플래시 등에 의한 빛 반사나 그늘로 글자가 잘 안 보이는 경우 정확한 Key/Value 추출이 어려울 수 있습니다.
* 흑백/컬러 이미지 모두 결과 분석이 가능하지만, 정확한 분석에는 컬러 이미지를 권장합니다.
* 세로 방향 카드의 경우, 카드 번호와 유효 기간이 정방향이 되도록 이미지를 준비하세요.
* 신용카드 분석 이미지 예시
...
* **신분증 주요 데이터 추출 및 분석**
    * 신분증의 종류(주민등록증/운전면허증/여권)에 따른 주요 데이터는 Key/Value 한 쌍으로 분석되며, 주요 데이터의 신뢰 값(confidence)을 제공합니다.
* **신분증 진위 확인**
    * 신분증(주민등록증, 운전면허증) 이미지 파일에서 추출한 결과를 바탕으로 신분증의 진위를 확인할 수 있습니다.
* **분석 결과 다운로드**
...
* 고객은 OCR 서비스에서 안전한 인식 결과 데이터를 제공받기 위해 권장하는 전송 방식(전용선, IPsec VPN 등)을 채택해야 합니다.
* 고객은 OCR 서비스로 인식된 정보를 저장/보관/관리할 때 개인정보 보호법 등 관련 법령을 준수해야 합니다.
* 회사는 고객이 위에서 정한 기술적, 관리적 조치를 모두 갖추고 있는지 확인이 필요한 경우 고객에게 증빙을 요청할 수 있습니다.
* 위와 같은 사항은 OCR 서비스로 고객이 수집/이용하는 정보가 중요한 정보에 해당하기에 고객에게 요청하는 사항입니다.<br>당사는 고객의 요청에 따라 수탁자로서 위탁받은 범위 내에서 정보를 처리하며, 고객은 정보처리의 주체로서 위 사항의 이행을 보증하고 위반으로 인하여 발생하는 정보주체, 규제기관 등에 대한 모든 책임을 부담하는 것을 확인합니다.
