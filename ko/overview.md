...
* **신뢰값 제공**
    * 이미지로 부터 추출한 텍스트의 신뢰값(confidence)을 제공합니다.
* **분석 결과 다운로드**
    * 이미지 파일에서 추출한 결과를 JSON 파일로 다운로드할 수 있습니다.
...
* 이미지 권장 사항
    * 평평한 곳에서 최대한 바르게 펴진 상태로 촬영된 이미지를 사용하세요.
    * 사각 형태의 가득 찬 이미지로 인식하세요.
    * 카메라 플래시 등으로 인한 빛 반사나 그늘로 인해 글자가 잘 안 보이는 경우 정확한 문자 추출이 어려울 수 있습니다.
    * 흑백/컬러 이미지에 대해 결과 분석이 가능하지만, 정확한 분석을 위해서 컬러 이미지를 권장합니다.
    * General OCR은 한국어/영어에 한해 분석 결과를 제공합니다.
